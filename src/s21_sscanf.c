...
int s21_sscanf(const char* str_orig, const char* format, ...) {
  Keys keys = {0};
...
char* str_mem_alloc(Keys* keys, const char* str_orig) {
  char* str = calloc(s21_strlen(str_orig) + 1, sizeof(char));
  if (!str) {
    printf("Memory allocation error function <str_mem_alloc>\n");
    keys->mem_error++;
  } else {
    s21_memcpy(str, str_orig, s21_strlen(str_orig));
    str[s21_strlen(str_orig)] = '\0';
  }
  return str;
}

void read_format(Spec* spec, char* str, const char* format, int* f_p_str) {
  spec->buffer = S21_NULL;
  spec->buffer = malloc(((int)s21_strlen(format) + 1) * sizeof(char));
  if (spec->buffer) {
    ...
    *p_buffer = '\0';
    spec->f_read_format++;
    if (s21_strncmp(sscanf_strtok(strtok_str(f_p_str, str), spec), spec->buffer,
                    s21_strlen(spec->buffer)) == 0) {
      spec->exit_flag = 0;
    } else {
      spec->exit_flag = 2;
    }
...
void sscanf_get_specifier(const char* format, Spec* spec, char* str,
  ...
  if (*format == '%') {
    spec->found = 0;
    spec->f_percent++;
    char* s_percent = sscanf_strtok(strtok_str(f_pars, str), spec);
    if (!s_percent) {
    } else {
      if (*s_percent != '%') {
        spec->skip_percent = 2;
...
int exit_condition(const char* str) {
  int result = 0;
  result = (s21_strlen(str) == 1 &&
            (*str == '-' || *str == '+' || *str == '.' || *str == 'e' ||
             *str == 'E' || *str == 'x' || *str == 'X'));
  result += (s21_strlen(str) > 1 && (*str == '-' && *(str + 1) == '-'));
  result += (s21_strlen(str) > 1 && (*str == '+' && *(str + 1) == '-'));
  result += (s21_strlen(str) > 1 && (*str == '-' && *(str + 1) == '+'));
  result += (s21_strlen(str) > 1 && (*str == '+' && *(str + 1) == '+'));
  result += (s21_strlen(str) == 2 && (*str == '-' && *(str + 1) == '.'));
  result += (s21_strlen(str) == 2 && (*str == '+' && *(str + 1) == '.'));
  result += (s21_strlen(str) == 2 && (*str == '+' && *(str + 1) == 'x'));
  result += (s21_strlen(str) == 2 && (*str == '+' && *(str + 1) == 'X'));
  result += (s21_strlen(str) == 2 && (*str == '-' && *(str + 1) == 'x'));
  result += (s21_strlen(str) == 2 && (*str == '-' && *(str + 1) == 'X'));
  result += (s21_strlen(str) > 1 && (*str == '.' && *(str + 1) == '-'));
  result += (s21_strlen(str) > 1 && (*str == '.' && *(str + 1) == '+'));
  result += (s21_strlen(str) > 1 && (*str == '0' && *(str + 1) == 'x') &&
             *(str + 2) == 'X');
  result += (s21_strlen(str) > 1 && (*str == '0' && *(str + 1) == 'X') &&
             *(str + 2) == 'x');
  result += (s21_strlen(str) > 1 && (*str == 'e' || *str == 'E'));
...
void sscanf_handle_specifier(va_list* args, Spec* spec, char* str,
  ...
  if (!spec->n) {
    ...
    if (ex_cond || !s21_strlen(tmp)) spec->exit_flag = 2;
  }
...
void sscanf_handle_i_int(va_list* args, Spec* spec, const char* str) {
  ...
  s21_size_t len_int_str = 0;
  const char* p_buff_2 = str;
  char* p_buff = S21_NULL;
  ...
  if (*p_buff_2 == 'x' || *p_buff_2 == 'X') {
    ...
    if (!s21_strchr("0123456789", *p_buff_2)) hex_char = 1;
    if (len_int_str)
      p_buff = s21_strtok((char*)str, &str[len_int_str + (math ? 1 : 0)]);
  }
  if (!flag_x && !hex_char) {
...
s21_size_t len_int(const char* str) {
  ...
  s21_size_t result = 0;
  int flag = 0;
  while (str != S21_NULL && *str != '\0' && !flag) {
    if (s21_strchr(tokens, *str))
      result++;
    else if (!s21_strchr(tokens, *str))
      flag = 1;
    str++;
  }
  return result;
...
void sscanf_handle_double(va_list* args, Spec* spec, const char* str) {
  long double num = 0;
  if (spec->s21_infinity) {
    if (spec->s21_infinity == 1) num = INFINITY;
    if (spec->s21_infinity == 2) num = -INFINITY;
  } else if (spec->s21_nan) {
    num = NAN;
  } else {
    const char* p_buff = str;
...
void sscanf_handle_string(va_list* args, Spec* spec, const char* str) {
  if (spec->length_l) {
    ...
    s21_strncpy(strg, str, s21_strlen(str) + 1);
  }
  spec->return_value++;
}

void sscanf_handle_n(va_list* args, Spec* spec) {
  if (spec->length_L || spec->length_l == 2) {
...
char* sscanf_strtok(char* str, Spec* spec) {
  ...
  if (!flag_next) {
    if (!(spec->c && !spec->prev_ws) && (!spec->buffer || spec->prev_ws)) {
      spec->index_n += s21_strspn(nextToken, "\v\t\r\n\f ", "1");
      nextToken += s21_strspn(nextToken, "\v\t\r\n\f ", "1");
    }
    spec->pars_count++;
    if (*nextToken == '\0') {
      nextToken = S21_NULL;
      ...
      token = nextToken;
      s21_size_t add = strtok_parser_string(nextToken, spec);
      spec->index_n += add;
      nextToken += add;
      if (*nextToken == '\0') {
        nextToken = S21_NULL;
...
s21_size_t strtok_parser_string(char* nextToken, Spec* spec) {
  int condition = 1;
  s21_size_t width = (s21_size_t)spec->width;
  s21_size_t string = length_string(spec, nextToken);
...
char* strtok_str(int* flag_parser_str, char* str) {
  return !((*flag_parser_str)++) ? str : S21_NULL;
}

s21_size_t length_string(Spec* spec, char* nextToken) {
  s21_size_t result = 0;
  if (spec->s) result = s21_strcspn(nextToken, "\v\t\r\n\f ");
  if (spec->d) result = s21_strspn(nextToken, "+-1234567890", "-+");
  if (spec->i)
    result = s21_strspn(nextToken, "+-xX0123456789abcdefABCDEF", "+-xX");
  if (spec->f || spec->e || spec->E || spec->g || spec->G) {
    result = s21_strspn(nextToken, "-+1234567890.eE", ".eE+");
    if (!result || (result == 1 && (*nextToken == '-' || *nextToken == '+'))) {
      result = find_inf(nextToken);
      if (result) {
        spec->s21_infinity = 1;
        if (*nextToken == '-') spec->s21_infinity = 2;
      }
    }
    if (!result || (result == 1 && (*nextToken == '-' || *nextToken == '+'))) {
      result = find_nan(nextToken);
      if (result) spec->s21_nan = 1;
    }
  }
  if (spec->c) result++;
  if (spec->x || spec->X)
    result = s21_strspn(nextToken, "+-xX1234567890abcdefABCDEF", "+-xX");
  if (spec->o) result = s21_strspn(nextToken, "+-12345670", "+-");
  if (spec->u) result = s21_strspn(nextToken, "+-1234567890", "+-");
  if (spec->p)
    result = s21_strspn(nextToken, "+-xX0123456789abcdefABCDEF", "+-xX");
  if (spec->f_ws) {
    result = s21_strspn(nextToken, "\v\t\r\n\f ", "1");
    spec->f_ws = 0;
  }
  if (spec->f_percent) {
    result = s21_strspn(nextToken, "%", "%");
    spec->f_percent = 0;
  }
  if (spec->f_read_format) {
    char* p = s21_strstr(nextToken, spec->buffer);
    if (p == nextToken) {
      result = s21_strlen(spec->buffer);
    }
  }
  return result;
}

s21_size_t find_inf(char* str) {
  char* p = str;
  int n = 4;
  s21_size_t flag_sign = 0;
  s21_size_t flag_inf = 0;
  while (*p != '\0' && n != 0) {
...
s21_size_t find_nan(char* str) {
  char* p = str;
  int n = 4;
  s21_size_t flag_sign = 0;
  s21_size_t flag_nan = 0;
  while (*p != '\0' && n != 0) {
    if ((*p == '+' || *p == '-') && n == 4) flag_sign++;
