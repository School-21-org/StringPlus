...
void int_conditions(const Spec* spec, int* has_sign, char* str, int* index,
                    char* p_buff) {
  int size = (int)s21_strlen(p_buff);
  ...
  if (spec->precision_direct > size - *has_sign) {
    ...
    int n = spec->precision_direct - (int)s21_strlen(p_buff) - *has_sign;
    while (n) {
      str[*index] = '0';
      *index += 1;
      n--;
    ...
    if ((int)s21_strlen(p_buff) == 1 && *p_buff == '0' && spec->has_precision &&
...
void uns_int_conditions(char* p_buff, const Spec* spec, char* str, int* index,
  ...
  if (!(spec->precision_direct > (int)s21_strlen(p_buff) && spec->o) &&
      has_sign)
    fill_0x(spec, p_buff, str, index, x);

  if (spec->precision_direct > (int)s21_strlen(p_buff)) {
    int n = spec->precision_direct - (int)s21_strlen(p_buff);
    while (n) {
      str[*index] = '0';
      *index += 1;
      n--;
...
void write_str(Spec* spec, char* str, char* buffer, int* index) {
  int s_size = (int)s21_strlen(buffer);
...
void handle_ptr(va_list* args, Spec* spec, char* str, int* index) {
  ...
  if (i == S21_NULL) {
    s21_strncat(p_buff, "(nil)", 5);
    nil_flag = 1;
  } else
    unsigned_int_to_str((unsigned long long int*)&i, p_buff, 16, 'x');
  int size = (int)s21_strlen(p_buff) + 2;
  if (!nil_flag) {
    ...
    if (size > (int)s21_strlen(p_buff)) {
      int n = size - 2 - (int)s21_strlen(p_buff);
      while (n) {
        str[*index] = '0';
        *index += 1;
        n--;
...
void handle_floating_point(va_list* args, Spec* spec, char* str, int* index) {
  ...
  if (buff != NULL) {
    ...
    if (isnan(i)) {
      ...
      int size = 0;
      if (i < 0) {
        s21_strncpy(p_inf_str, "-inf", 4);
        size = 4;
      } else {
        s21_strncpy(p_inf_str, "inf", 3);
        size = 3;
      }
      handle_width_before(spec, str, index, size);
