...
- go to `src` folder
- run `make my_string.a` to build a library
- now you can use `my_string.a` library and compile with any required program.

## Unit tests
- go to `src` folder
- run `make` to build all targets including (my_string.a, test, gcov_report) / or run `make test` to run tests separately
- tests results will be printed to stdout

## LCOV report

- go to `src` folder
- run `make` to build all targets including (my_string.a, test, gcov_report) / or run `make gcov_report` to run tests with gcov separately
